...
{
    coeffs_.readEntry("frequency", freq_);
    coeffs_.readEntry("fieldNames", fieldNames_);
...
bool momentumFvOptionSource::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        nLoc_ = coeffs_.get<scalar>("nLoc");
        coeffs_.readEntry("frequency", freq_);
        coeffs_.readEntry("area", Ainj_);
        coeffs_.readEntry("velocity", velocity_);
        coeffs_.readEntry("nuAir", nuAir_);
        coeffs_.readEntry("particleDiameter", dp_);
